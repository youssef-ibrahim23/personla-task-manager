...
class _AllTasksViewState extends ConsumerState<AllTasksView> {
  ...
  Widget build(BuildContext context) {
    ...
    final taskCount = widget.tasks?.length ?? 0;
    final completedCount = taskCount > 0
        ? widget.tasks!.where((t) => t.endDate.isBefore(DateTime.now())).length
        : 0;
  ...
  Widget _buildMainContent(
    ...
    return Positioned.fill(
      ...
      child: Container(
        ...
        child: isLoading
            ? TasksShimmer(length: widget.tasks?.length ?? 10)
            : widget.tasks == null || widget.tasks!.isEmpty
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      AppLocalizations.of(context)!.swipe_to_delete_hint,
                      style: TextStyle(
  ...
  Widget _buildStatsFloatingActionButton(BuildContext context, ThemeData theme, int taskCount) {
    ...
    return Padding(
      ...
      child: FloatingActionButton.extended(
        onPressed: () {
          _showStatisticsDialog(context, theme);
        },
        backgroundColor: theme.colorScheme.primary,
  ...
  Future<void> _showStatisticsDialog(BuildContext context, ThemeData theme) async {
    final completedTasks = widget.tasks?.where((t) => t.endDate.isBefore(DateTime.now())).length ?? 0;
    final pendingTasks = (widget.tasks?.length ?? 0) - completedTasks;
    final highPriority = widget.tasks?.where((t) => t.priority.toLowerCase().contains('high')).length ?? 0;
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          '${widget.tasks!.length > 0 ? ((completedTasks / widget.tasks!.length) * 100).toInt() : 0}%',
                          style: TextStyle(
                            color: theme.colorScheme.primary,
                    ...
                    LinearProgressIndicator(
                      value: widget.tasks!.length > 0 ? completedTasks / widget.tasks!.length : 0,
